...
begin
	args = filter(x -> !isempty(x), [path, url])
	
	command = `python3 $args`
	show(command)
	md"Execute! $(@bind enable_exec CheckBox())"	
end

# ╔═╡ ee6712a6-71b4-431a-9710-79908beff350
if enable_exec
	@time run(pipeline(command, stdout="log.txt", stderr="log.txt", append=true), wait=false) # exec async to not have a hanging cell
end
# a log.txt is created in the loc of the notebook and can be streamed/followed
# seems impossible to stream text files in arc browser tho
